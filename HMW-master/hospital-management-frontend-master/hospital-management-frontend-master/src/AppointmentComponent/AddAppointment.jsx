...
const AddAppointment = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/users/doctor/all")
  ...
  const saveAppointment = async (event) => {
    ...
    await fetch("http://localhost:5000/api/appointments/patient/add", {
      method: "POST",
      headers: {
    ...
    navigate(`/patient/appointments/${patient.id}`);
  };

  if (!patient) {
    return (
      <h1 className="mt-2">Please Login as Patient to get Add Appointment </h1>
    );
  }
